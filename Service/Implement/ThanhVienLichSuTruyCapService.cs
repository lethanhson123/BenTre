namespace Service.Implement
{
	...
	, IThanhVienLichSuTruyCapService
	{
		...
		public override void Initialization(ThanhVienLichSuTruyCap model)
		{
            ...
            if (model.NgayTruyCap == null)
			{
				model.NgayTruyCap = GlobalHelper.InitializationDateTime;
			}
		}
        public override ThanhVienLichSuTruyCap Save(ThanhVienLichSuTruyCap model)
        {
            if (model.ID > 0)
            {
                Update(model);
        ...
        public override async Task<ThanhVienLichSuTruyCap> SaveAsync(ThanhVienLichSuTruyCap model)
        {
            if (model.ID > 0)
            {
        ...
        public override async Task<List<ThanhVienLichSuTruyCap>> GetBySearchStringToListAsync(string searchString)
		{
			...
			if (!string.IsNullOrEmpty(searchString))
			{
				...
				result = result.OrderByDescending(item => item.NgayTruyCap).ToList();
			}
			return result;
		}
		public virtual async Task<List<ThanhVienLichSuTruyCap>> GetByBatDau_KetThucToListAsync(DateTime batDau, DateTime ketThuc)
		{
			...
			result = await GetByCondition(item => item.NgayTruyCap >= batDau && item.NgayTruyCap <= ketThuc).ToListAsync();
			result = result.OrderByDescending(item => item.NgayTruyCap).ToList();
			return result;
		}
