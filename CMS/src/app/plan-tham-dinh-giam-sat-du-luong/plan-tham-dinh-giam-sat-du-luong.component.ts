...
@Component({
  selector: 'app-plan-tham-dinh-giam-sat-du-luong',
  templateUrl: './plan-tham-dinh-giam-sat-du-luong.component.html',
  styleUrls: ['./plan-tham-dinh-giam-sat-du-luong.component.css']
})
export class PlanThamDinhGiamSatDuLuongComponent implements OnInit {
  ...
  PlanThamDinhSearch() {
    if (this.PlanThamDinhService.BaseParameter.SearchString.length > 0) {
      ...
      this.PlanThamDinhService.IsShowLoading = true;
      this.PlanThamDinhService.BaseParameter.ParentID = environment.PlanTypeIDGiamSatDuLuong;
  ...
  PlanThamDinhAdd(ID: number) {
    ...
    this.PlanThamDinhService.GetByIDAsync().subscribe(
      res => {
        ...
        dialogConfig.width = environment.DialogConfigWidth;
        dialogConfig.data = { ID: ID };
        const dialog = this.dialog.open(PlanThamDinhDetailGiamSatDuLuong001Component, dialogConfig);
        dialog.afterClosed().subscribe(() => {
          this.PlanThamDinhSearch();
        });
        this.PlanThamDinhService.IsShowLoading = false;
