...
@Component({
  selector: 'app-plan-tham-dinh-chuoi-cung-ung-an-toan-thuc-pham-ke-hoach-lay-mau',
  templateUrl: './plan-tham-dinh-chuoi-cung-ung-an-toan-thuc-pham-ke-hoach-lay-mau.component.html',
  styleUrls: ['./plan-tham-dinh-chuoi-cung-ung-an-toan-thuc-pham-ke-hoach-lay-mau.component.css']
})
export class PlanThamDinhChuoiCungUngAnToanThucPhamKeHoachLayMauComponent implements OnInit {
  ...
  PlanThamDinhSearch() {
    if (this.PlanThamDinhService.BaseParameter.SearchString.length > 0) {
      ...
      this.PlanThamDinhService.IsShowLoading = true;
      this.PlanThamDinhService.BaseParameter.ParentID = environment.PlanTypeIDChuoiCungUngKeHoachLayMau;
  ...
  PlanThamDinhDanhMucLayMauSearch(i: number) {
    ...
    this.PlanThamDinhDanhMucLayMauService.BaseParameter.IsGoiY = false;
    this.PlanThamDinhDanhMucLayMauService.GetByParentID_IsGoiYToListAsync().subscribe(
      res => {
  ...
  PlanThamDinhAdd(ID: number) {
    ...
    this.PlanThamDinhService.GetByIDAsync().subscribe(
      res => {
        ...
        this.PlanThamDinhService.FormData.ParentID = environment.PlanTypeIDChuoiCungUngKeHoachLayMau;
        const dialogConfig = new MatDialogConfig();
        dialogConfig.disableClose = true;
        dialogConfig.autoFocus = true;
        dialogConfig.width = environment.DialogConfigWidth;
