...
@Component({
  selector: 'app-plan-tham-dinh',
  templateUrl: './plan-tham-dinh.component.html',
  styleUrls: ['./plan-tham-dinh.component.css']
})
export class PlanThamDinhComponent implements OnInit {
  ...
  PlanThamDinhSearch() {
    if (this.PlanThamDinhService.BaseParameter.SearchString.length > 0) {
      ...
      this.PlanThamDinhService.IsShowLoading = true;
      this.PlanThamDinhService.BaseParameter.ParentID = environment.PlanTypeIDThamDinhATTP;
  ...
  PlanThamDinhCompaniesAdd(ID: number) {
    ...
    this.PlanThamDinhService.GetByIDAsync().subscribe(
      res => {
        ...
        dialogConfig.width = environment.DialogConfigWidth;
        dialogConfig.data = { ID: ID };
        const dialog = this.dialog.open(PlanThamDinhCompaniesComponent, dialogConfig);
        dialog.afterClosed().subscribe(() => {
        });
        this.PlanThamDinhService.IsShowLoading = false;
      },
