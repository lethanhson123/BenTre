...
@Component({
  selector: 'app-plan-tham-dinh-an-toan-thuc-pham-sau-thu-hoach',
  templateUrl: './plan-tham-dinh-an-toan-thuc-pham-sau-thu-hoach.component.html',
  styleUrls: ['./plan-tham-dinh-an-toan-thuc-pham-sau-thu-hoach.component.css']
})
export class PlanThamDinhAnToanThucPhamSauThuHoachComponent implements OnInit {
  ...
  PlanThamDinhSearch() {
    if (this.PlanThamDinhService.BaseParameter.SearchString.length > 0) {
      ...
      this.PlanThamDinhService.IsShowLoading = true;
      this.PlanThamDinhService.BaseParameter.ParentID = environment.PlanTypeIDAnToanThucPhamSauThuHoach;
  ...
  PlanThamDinhAdd(ID: number) {
    ...
    this.PlanThamDinhService.GetByIDAsync().subscribe(
      res => {
        ...
        dialogConfig.width = environment.DialogConfigWidth;
        dialogConfig.data = { ID: ID };
        const dialog = this.dialog.open(PlanThamDinhDetailAnToanThucPhamSauThuHoach001Component, dialogConfig);
        dialog.afterClosed().subscribe(() => {
          this.PlanThamDinhSearch();
        });
        this.PlanThamDinhService.IsShowLoading = false;
