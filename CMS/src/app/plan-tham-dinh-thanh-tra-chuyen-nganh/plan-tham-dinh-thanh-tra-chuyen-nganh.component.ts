...
@Component({
  selector: 'app-plan-tham-dinh-thanh-tra-chuyen-nganh',
  templateUrl: './plan-tham-dinh-thanh-tra-chuyen-nganh.component.html',
  styleUrls: ['./plan-tham-dinh-thanh-tra-chuyen-nganh.component.css']
})
export class PlanThamDinhThanhTraChuyenNganhComponent implements OnInit {
  ...
  PlanThamDinhSearch() {
    if (this.PlanThamDinhService.BaseParameter.SearchString.length > 0) {
      ...
      this.PlanThamDinhService.IsShowLoading = true;
      this.PlanThamDinhService.BaseParameter.ParentID = environment.PlanTypeIDThanhTraChuyenNganh;
          ...
          if (this.PlanThamDinhService.List) {
            if (this.PlanThamDinhService.List.length > 0) {
              for (let i = 0; i < this.PlanThamDinhService.List.length; i++) {
                this.PlanThamDinhDistrictDataSearch(i);
  ...
  PlanThamDinhAdd(ID: number) {
    ...
    this.PlanThamDinhService.GetByIDAsync().subscribe(
      res => {
        ...
        this.PlanThamDinhService.FormData.ParentID = environment.PlanTypeIDThanhTraChuyenNganh;
        const dialogConfig = new MatDialogConfig();
        dialogConfig.disableClose = true;
        dialogConfig.autoFocus = true;
        dialogConfig.width = environment.DialogConfigWidth;
        dialogConfig.data = { ID: ID };
        const dialog = this.dialog.open(PlanThamDinhDetailThanhTraChuyenNganhComponent, dialogConfig);
        dialog.afterClosed().subscribe(() => {
          this.PlanThamDinhSearch();
        });
        this.PlanThamDinhService.IsShowLoading = false;
