...
@Component({
  selector: 'app-co-so-company-info-don-vi-dong-goi-detail',
  templateUrl: './co-so-company-info-don-vi-dong-goi-detail.component.html',
  styleUrls: ['./co-so-company-info-don-vi-dong-goi-detail.component.css']
})
export class CoSoCompanyInfoDonViDongGoiDetailComponent implements OnInit {
  ...
  constructor(
    ...
    public dialogRef: MatDialogRef<CoSoCompanyInfoDonViDongGoiDetailComponent>,
  ...
  GetCompanyInfo(){
    if(this.CompanyInfoDonViDongGoiService.FormData.ID==0){
      ...
      this.CompanyInfoService.GetByIDAsync().subscribe(
        res => {
          this.CompanyInfoService.FormData = res as CompanyInfoDonViDongGoiDocuments;
  ...
  ThanhVienAdd(ID: number) {
    ...
    this.ThanhVienService.GetByIDAsync().subscribe(
      res => {
        ...
        const dialog = this.dialog.open(CoSoThanhVienDetailComponent, dialogConfig);
        dialog.afterClosed().subscribe(() => {
