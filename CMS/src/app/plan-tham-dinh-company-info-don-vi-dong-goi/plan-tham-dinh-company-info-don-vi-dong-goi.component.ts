...
@Component({
  selector: 'app-plan-tham-dinh-company-info-don-vi-dong-goi',
  templateUrl: './plan-tham-dinh-company-info-don-vi-dong-goi.component.html',
  styleUrls: ['./plan-tham-dinh-company-info-don-vi-dong-goi.component.css']
})
export class PlanThamDinhCompanyInfoDonViDongGoiComponent implements OnInit {
  ...
  DateBatDau(value) {
    this.PlanThamDinhService.BaseParameter.BatDau = new Date(value);
  }
  DateKetThuc(value) {
    this.PlanThamDinhService.BaseParameter.KetThuc = new Date(value);
  }
  ...
  PlanThamDinhSearch() {
    if (this.PlanThamDinhService.BaseParameter.SearchString.length > 0) {
      ...
      this.PlanThamDinhService.IsShowLoading = true;
      this.PlanThamDinhService.BaseParameter.ParentID = environment.PlanTypeIDDangKyMaDongGoi;
      this.PlanThamDinhService.BaseParameter.Active = true;
      this.PlanThamDinhService.GetByParentID_SearchString_NgayBatDau_NgayKetThuc_ActiveToListAsync().subscribe(
        res => {
          ...
          if (this.PlanThamDinhService.List) {
            if (this.PlanThamDinhService.List.length > 0) {
              for (let i = 0; i < this.PlanThamDinhService.List.length; i++) {
                ...
                this.PlanThamDinhCompaniesSearch(i);
              }
            }
          }
  ...
  PlanThamDinhAdd(ID: number) {
    ...
    this.PlanThamDinhService.GetByIDAsync().subscribe(
      res => {
        ...
        dialogConfig.width = environment.DialogConfigWidth;
        dialogConfig.data = { ID: ID };
        const dialog = this.dialog.open(PlanThamDinhDetailCompanyInfoDonViDongGoiComponent, dialogConfig);
        dialog.afterClosed().subscribe(() => {
          this.PlanThamDinhSearch();
        });
        this.PlanThamDinhService.IsShowLoading = false;
  ...
  PlanThamDinhCompaniesAdd(ID: number) {
    ...
    this.PlanThamDinhService.GetByIDAsync().subscribe(
      res => {
        ...
        dialogConfig.width = environment.DialogConfigWidth;
        dialogConfig.data = { ID: ID };
        const dialog = this.dialog.open(PlanThamDinhCompaniesCompanyInfoDonViDongGoiComponent, dialogConfig);
        dialog.afterClosed().subscribe(() => {
        });
        this.PlanThamDinhService.IsShowLoading = false;
      },
  ...
  MailAdd(ID: number) {
    ...
    this.MailService.AnToanThucPhamThamDinhThongBaoByPlanThamDinhIDAsync().subscribe(
      res => {
        this.PlanThamDinhService.IsShowLoading = false;
      },
      err => {
